...
import com.job.userservice.exception.UnauthorizedException;
import com.job.userservice.repository.JobApplicationRepository;
...
public class JobApplicationServiceImplementation implements JobApplicationService {
	...
	@Override
	@Transactional
	public JobApplication applyForJob(User users, Long jobId) {
		  String email = jwtUtility.generateToken(users.getEmail()); // Get email from JWT
		    User user = userRepository.findByEmail(email);
		    if (user == null || !user.getId().equals(users.getId())) {
		        throw new IllegalStateException("Unauthorized: User not logged in.");
		    }
	        ...
	        JobApplication jobApplication = new JobApplication(user,JobApplication.ApplicationStatus.PENDING,LocalDateTime.now());
	...
	public List<JobApplication> getApplicationsByUserId(User user) {
		if (!userRepository.existsById(user.getId())) {
            throw new ResourceNotFoundException("No Applications found with email: " +user.getEmail());
        }
	...
	@Override
	@Transactional
	public JobApplication updateApplicationStatus(Long jobApplicationId, JobApplication.ApplicationStatus status, User recruiter) {
	    JobApplication jobApplication = jobApplicationRepository.findById(jobApplicationId)
	            .orElseThrow(() -> new ResourceNotFoundException("Application not found with ID: " + jobApplicationId));

	    // Ensure only the recruiter can update the status
